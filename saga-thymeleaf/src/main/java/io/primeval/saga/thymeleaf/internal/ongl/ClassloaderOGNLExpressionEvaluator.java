...
import io.primeval.saga.thymeleaf.internal.ThymeleafTemplateEngineImpl;
...
public class ClassloaderOGNLExpressionEvaluator implements IStandardVariableExpressionEvaluator {

    public static final ClassloaderOGNLExpressionEvaluator INSTANCE = new ClassloaderOGNLExpressionEvaluator(true);
    private static final Logger LOGGER = LoggerFactory.getLogger(ClassloaderOGNLExpressionEvaluator.class);

    private static final String EXPRESSION_CACHE_TYPE_OGNL = "ognl";

    private static Map<String, Object> CONTEXT_VARIABLES_MAP_NOEXPOBJECTS_RESTRICTIONS = Collections
            .singletonMap(
    ...
    public ClassloaderOGNLExpressionEvaluator(final boolean applyOGNLShortcuts) {
    ...
    private static Object evaluate(
        ...
        try {
            ...
            final ComputedOGNLExpression parsedExpression = obtainComputedOGNLExpression(configuration, expression, exp,
                    applyOGNLShortcuts);

            ClassLoader classLoader = (ClassLoader) context.getVariable(ThymeleafTemplateEngineImpl.CLASSLOADER_VAR);
            ...
            if (parsedExpression.mightNeedExpressionObjects) {
                ...
                if (expContext.getRestrictVariableAccess()) {
                    contextVariablesMap.put(OGNLContextPropertyAccessor.RESTRICT_REQUEST_PARAMETERS,
                            OGNLContextPropertyAccessor.RESTRICT_REQUEST_PARAMETERS);
                } else {
            ...
            // active or not...
            final ITemplateContext templateContext = (context instanceof ITemplateContext ? (ITemplateContext) context
                    : null);
            final Object evaluationRoot = (useSelectionAsRoot && templateContext != null
                    && templateContext.hasSelectionTarget() ? templateContext.getSelectionTarget() : templateContext);

            // Execute the expression!
            final Object result;
            try {
                result = executeExpression(classLoader, configuration, parsedExpression.expression, contextVariablesMap,
                        evaluationRoot);
            ...
            final IStandardConversionService conversionService = StandardExpressions
                    .getConversionService(configuration);
    ...
    private static ComputedOGNLExpression parseComputedOGNLExpression(
        ...
        ComputedOGNLExpression parsedExpression = (ComputedOGNLExpression) ExpressionCache.getFromCache(configuration,
                exp, EXPRESSION_CACHE_TYPE_OGNL);
    ...
    private static ComputedOGNLExpression parseExpression(final String expression, final boolean applyOGNLShortcuts)
        ...
        if (applyOGNLShortcuts) {
            ...
            if (parsedExpression != null) {
                return new ComputedOGNLExpression(new OGNLShortcutExpression(parsedExpression),
                        mightNeedExpressionObjects);
            }
        }
    ...
    private static Object executeExpression(
            ClassLoader classLoader, final IEngineConfiguration configuration, final Object parsedExpression,
