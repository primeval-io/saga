 ...
 * =============================================================================
 */
package io.primeval.saga.thymeleaf.internal.ongl;
...
final class OGNLShortcutExpression {
    ...
    Object evaluate(
        ...
        final ICache<ExpressionCacheKey, Object> expressionCache = (cacheManager == null ? null
                : cacheManager.getExpressionCache());
     ...
     *
     * @param propertyName
     *            the name of the property being accessed (we are looking for 'execInfo').
     * @param context
     *            the expression context, which should contain the expression objects.
     * @deprecated created (and deprecated) in 3.0.0 in order to support automatic conversion of calls to the 'execInfo' context variable (${execInfo}) into the
     *             'execInfo' expression object (${#execInfo}), which is its new only valid form. This method, along with the infrastructure for execInfo
     *             conversion in StandardExpressionUtils#mightNeedExpressionObjects(...) will be removed in 3.1.
     */
    @Deprecated
    ...
    private static Object getMapProperty(final String propertyName, final Map<?, ?> map) {
        ...
        /*
         * This method will try to mimic the behaviour of the ognl.MapPropertyAccessor class, with the exception that OGNLShortcutExpressions do not process
         * indexed map access (map['key']), only normal property map access (map.key), so indexed access will not be taken into account in this accessor method.
         *
         * The main reason for not implementing support for indexed map access in OGNLShortcutExpression is that in an indexed access expression in OGNL a
         * variable could be used as index instead of a literal (note that this is not allowed in SpringEL, but it is in OGNL), and resolving such index
         * variable or more complex expression would add quite a lot of complexity to this supposedly-simple mechanism. So in those cases, it is just better to
         * allow OGNL to do its job.
         */
    ...
    public static Object getListProperty(
        ...
        /*
         * This method will try to mimic the behaviour of the ognl.ListPropertyAccessor class, with the exception that OGNLShortcutExpressions do not process
         * indexed list access (list[3]), only access to the properties of the list object like 'size', 'iterator', etc.
         *
         * The main reason for not implementing support for indexed list access in OGNLShortcutExpression is similar to that of indexed map access (with the
         * difference that typical literal-based indexed access to lists is based on numeric literals instead of text literals).
         */
    ...
    public static Object getArrayProperty(
        ...
        /*
         * This method will try to mimic the behaviour of the ognl.ArrayPropertyAccessor class, with the exception that OGNLShortcutExpressions do not process
         * indexed array access (array[3]), only access to the properties of the array object, namely 'length'.
         *
         * The main reason for not implementing support for indexed array access in OGNLShortcutExpression is similar to that of indexed map access (with the
         * difference that typical literal-based indexed access to arrays is based on numeric literals instead of text literals).
         */
    ...
    public static Object getEnumerationProperty(
            final ICache<ExpressionCacheKey, Object> expressionCache, final String propertyName,
            final Enumeration enumeration) {

        /*
         * This method will try to mimic the behaviour of the ognl.EnumerationPropertyAccessor class, with the exception that OGNLShortcutExpressions do not
         * process indexed array access (array[3]), only access to the properties of the enumeration object.
         */
    ...
    public static Object getIteratorProperty(
        ...
        /*
         * This method will try to mimic the behaviour of the ognl.IteratorPropertyAccessor class, with the exception that OGNLShortcutExpressions do not
         * process indexed iterator access (array[3]), only access to the properties of the iterator object.
         */
    ...
    /*
     * This exception signals that the OGNLShortcutExpression mechanism is not applicable for the current expression, and therefore the
     * OGNLVariableExpressionEvaluator should default to standard pure-OGNL evaluation.
     *
     * Most common reason for this is the existence of a custom property accessor registered in OGNL for accessing the properties of one of the objects involved
     * in the expression, which behaviour (the custom property accessor's) cannot be replicated by OGNLShortcutExpressions.
     */
