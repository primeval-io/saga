...
public final class MimeParse {
     ...
     * ('application', '*', {'q', '0.5'})
     * 
     * In addition this function also guarantees that there is a value for 'q' in the params dictionary, filling it in
     * with a proper default if necessary.
     * 
     * @param range
     */
    ...
    private static float toFloat(String s, float def) {
        if (s == null) {
            return def;
        }
        Float ff = Floats.tryParse(s);
        float f = ff == null ? def : ff.floatValue();
        return f;
    }
    ...
    /**
     * Find the best match for a given mime-type against a list of ranges that have already been parsed by
     * parseMediaRange(). Returns the 'q' quality parameter of the best match, 0 if no match was found. This function
     * bahaves the same as quality() except that 'parsed_ranges' must be a list of parsed media ranges.
     * 
     * @param mimeType
     * @param parsedRanges
     * @return
    ...
    /**
     * Takes a list of supported mime-types and finds the best match for all the media-ranges listed in header. The
     * value of header must be a string that conforms to the format of the HTTP Accept: header. The value of 'supported'
